...
salt_remover = SaltRemover(
    defnFilename=os.path.join(current_path, "files", "Salts.txt")
)


def mp_featurize(mol, feat_info):
    mol.featurize(feat_info)
    if not np.all(np.isfinite(mol.features_values)) or np.any(
        np.isnan(mol.features_values)
    ):
        mol.mol_block = None
...
class SMol:
    def __init__(self, source, endpoints=None, id=None, standardization=1):
        ...
        if standardization == 1:
            ...
            self.mol_block = (
                Chem.MolToMolBlock(rdk_mol) if rdk_mol is not None else None
            )
        else:
    ...
    def featurize(self, features_info):
        ...
        for feature_info in features_info:
            ...
            features_values.extend(
                feat_dict[feature_info["type"]](self.rmol, feature_info)
            )
    ...
    def features(self):
        return {
            ...
            for feature_name, features_value in zip(
                self.features_names, self.features_values
            )
        }
