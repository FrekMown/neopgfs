...
class ActionValueNetwork:
    """Neural Network representing Action-value approximation
    """
    ...
    def get_TD_update(self, s, delta_mat):
        ...
        v = delta_mat
        td_update[1]["W"] = np.dot(x.T, v) * 1.0 / s.shape[0]
        td_update[1]["b"] = np.sum(v, axis=0, keepdims=True) * 1.0 / s.shape[0]

        v = np.dot(v, W1.T) * dx
        td_update[0]["W"] = np.dot(s.T, v) * 1.0 / s.shape[0]
        td_update[0]["b"] = np.sum(v, axis=0, keepdims=True) * 1.0 / s.shape[0]
